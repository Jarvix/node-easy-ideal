    ...
    function call(action, paramaters, path, callback) {
        ...
        post(postdata, function (error, result) {
            ...
            if (output.Response && output.Response.Status === 'OK') {
                if (output.Response.Response && output.Respone.Response.Code !== undefined)
                    _lastTransactionCode = output.Respone.Response.Code;
                else
                    _lastTransactionCode = null;

                if (output.Response.Response && output.Respone.Response.TransactionID !== undefined)
                    _lastTransactionID = output.Respone.Response.TransactionID;
                else
